...
- **Grouped-deck side-card: click-to-toggle + no auto-open during multi-select.** Clicking an inventory row/card opens the grouped deck and clicking it again closes it; in multi-select the deck no longer pops open on each pick — the checkbox selects, the row/card body toggles the deck. Grid view is now iso with the table (per-card checkbox selects, card click toggles the deck) — `renderer/inventory.js`, `renderer/css/70-detail-misc.css`.
