// tz-country.js — offline IANA timezone → ISO 3166-1 alpha-2 country code.
//
// Used by the telemetry layer to derive a user's country WITHOUT any IP
// geolocation (no PII, no network call). The browser locale is tried first
// ("fr-FR" → "FR"); this map is the fallback when the locale carries no
// region subtag (e.g. plain "fr"), using Intl.DateTimeFormat's resolved
// timeZone ("Europe/Paris" → "FR").
//
// Derived from the IANA tz database (zone1970.tab). Canonical zones only;
// the function also resolves the multi-zone country prefixes
// (America/Argentina/*, America/Indiana/*, …) and a few common legacy
// aliases. Unknown zones return null — same graceful degradation as before.
...
const TZ_COUNTRY = {
  ...
  "America/Antigua": "AG", "America/Araguaina": "BR", "America/Aruba": "AW",
  ...
  "America/Chihuahua": "MX", "America/Ciudad_Juarez": "MX", "America/Costa_Rica": "CR",
  ...
  "America/Fortaleza": "BR", "America/Glace_Bay": "CA", "America/Goose_Bay": "CA",
  ...
  "America/Juneau": "US", "America/Kralendijk": "BQ", "America/La_Paz": "BO",
  ...
  "America/Nome": "US", "America/Noronha": "BR", "America/Nuuk": "GL",
  ...
  "Europe/Zurich": "CH",
  // common legacy alias still emitted by some runtimes
...
// Multi-zone countries expose sub-paths (America/Argentina/Buenos_Aires,
// America/Indiana/Indianapolis, …). Match by prefix so every sub-zone
// resolves without listing each one individually.
const TZ_PREFIX = {
...
/**
 * Resolve an IANA timezone to an ISO 3166-1 alpha-2 country code.
 * @param {string} tz e.g. "Europe/Paris"
 * @returns {string|null} "FR", or null when unknown.
 */
export function tzToCountry(tz) {
  if (!tz || typeof tz !== "string") return null;
  if (TZ_COUNTRY[tz]) return TZ_COUNTRY[tz];
  for (const prefix in TZ_PREFIX) {
    if (tz.startsWith(prefix)) return TZ_PREFIX[prefix];
  }
  return null;
}
