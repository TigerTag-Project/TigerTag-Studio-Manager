...
- **HTTP polling** — 2 s tick on `POST /detail` (matches the FlashForge mobile companion app cadence), capped exponential backoff on network errors (2 s → 30 s), idempotent reconnect on side-card open, full tear-down on close. Every poll is bridged through the Electron main process (Node `fetch`, no CORS) — direct renderer-side `fetch` was being blocked by Chromium's CORS preflight on `application/json`, which the FlashForge firmware doesn't handle.
- **Camera (MJPEG)** — full-width edge-to-edge `<img>` stream from `cameraStreamUrl` (typically `:8080/?action=stream`). The printer's mjpg-streamer accepts ONE concurrent client, so the side card now (a) appends a per-session cache-buster to the URL on every open, (b) explicitly tears the `<img>` down to `about:blank` on close (forces TCP FIN → printer frees its slot in milliseconds), and (c) on `error` event swaps in a graceful fallback view: the printer's product photo darkened, an overlay explaining the 1-stream limit, and a **Retry** button that bumps the cache-buster session and re-attempts. Status flapping during a session no longer triggers full re-renders, so the camera doesn't churn and neighbouring open sections (Request log, etc.) stay open.
- **5-slot matlStation grid** — `[Ext.] [1A] [1B] [1C] [1D]` honoring the FlashForge bay numbering convention. The Ext. slot maps to `indepMatlInfo` (the direct-feed bowden, dispatched as `ipdMsConfig_cmd` on save); the 4 matlStation bays map to `slotInfos[1..4]` (dispatched as `msConfig_cmd` with `slot:1-4`). Each slot has THREE visual states: **filled** (solid colour fill — `hasFilament: true`), **configured-but-empty** (4-px coloured ring drawn INSIDE the square via `box-shadow: inset` so layout stays identical across states; user can see at a glance which filament is *intended* for each empty bay), or **unconfigured** (rare — falls back to the existing grey hatch). The vendor row under each card is hidden when null — the `/detail` payload doesn't carry brand data, so we don't fabricate one. The "Change filament" bottom-sheet was trimmed to two brand options (`Generic` first, `FlashForge` second) since vendor is purely local UI metadata and isn't part of the wire payload.
- **Auto SN-prefix** — FlashForge's chassis label shows the serial as e.g. `MQQE9501368`, but the firmware's HTTP API authenticates against the prefixed form `SNMQQE9501368` (queryable via `~M115` on port 8899). The mobile companion app strips the prefix when displaying it, so anyone copying the SN off-screen ends up with the short form and gets `code:1, "SN is different"` on every poll. The driver now auto-prefixes `SN` when missing — a SN that already starts with `SN` is left untouched. Idempotent.
- **Request log (debug mode)** — collapsible section at the bottom of the side card, mirroring the Snapmaker WS log. Every poll pushes one outbound entry (`→ POST http://<ip>:8898/detail` row head + the exact wire body in the expanded view) and one inbound entry (`← code:0 detail · status, rightTemp, …` summary + full pretty-printed response). Click any row to expand the JSON; dedicated Pause / Clear toolbar; entries cap at 100 (FIFO). The user's expanded/collapsed state is now persisted across status flaps via `conn.logExpanded`, so the section doesn't snap shut on every offline → online transition.
- **i18n** — 4 new keys across the 9 locales for the camera fallback (`ffgCamFailMsg`, `ffgCamRetry`) and the slot empty label (`ffgSlotEmpty`).

#### UX — Inventory toolbar redesign
- **View selector moved BELOW the search bar** — was previously squeezed into the same row as the search input + brand/material/type filters, which made the buttons compete for horizontal space when filters were open. Now lives on its own dedicated row directly under the search, keeping its full width regardless of how wide the filters above end up.
- **Search input — clear button (✕)** — appears on the LEFT of the magnifier icon as soon as the input contains a value, hidden when empty. One click wipes the input, refocuses it, re-renders the inventory immediately. Same colour palette as the existing surface tokens, soft hover.

#### Internal
- **CORS bypass IPC** (`main.js`) — new `ipcMain.handle('ffg:http-post', …)` exposed via preload as `electronAPI.ffgHttpPost(url, body)`. Allowlisted to FlashForge `/detail` + `/control` paths (regex on the URL pathname) so a renderer compromise can't pivot it into a generic outbound HTTP proxy. Same `code:-1` / `code:-2` envelope shape as the Flutter monolith for parse / network errors so the renderer's existing error contract just works.
- **Defensive bridge fallback** — every renderer-side call wraps the IPC in a try/catch + null-bridge guard, surfacing failures as a synthetic `code:-2` payload with a clear message ("ffgHttpPost bridge unavailable (full app restart required)" when the user reloads the renderer without restarting main, "IPC failed — No handler registered" when the main process is stale, etc.). The Request log shows the failure verbatim so debugging connection issues is now self-explanatory.
- **Status-transition guards** — `ffgMergeStatus` no longer flags `statusChanged: true` on every error poll. It captures the previous status and only flags transitions (`offline → connecting`, `error → connected`, etc.). Eliminates the 2 s flicker that was rebuilding the entire side card every poll while the printer was unreachable.
